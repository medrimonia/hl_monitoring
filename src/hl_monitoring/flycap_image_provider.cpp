...
FlyCapImageProvider::FlyCapImageProvider(const Json::Value & v,
  ...
  try {
    ...
    while (true) {
      ...
      if (error == FlyCapture2::PGRERROR_ISOCH_BANDWIDTH_EXCEEDED) {
        ...
        std::cerr << HL_DEBUG + "Failed to start image capture: '"
                  << error.GetDescription() << "'";
        reconnectCamera();
      } else {
        break;
...
void FlyCapImageProvider::updatePacketProperties() {
  ...
  if (error != FlyCapture2::PGRERROR_OK) {
    std::ostringstream oss;
    oss << "FlyCapImageProvider::updatePacketProperties: failed to set packet size: "
        << error.GetType();
    {
...
void FlyCapImageProvider::updateBinning(unsigned int h_binning, unsigned int v_binning) {
  ...
  error = camera.GetGigEImageBinningSettings(&current_h_binning,
                                             &current_v_binning);
  if (error != FlyCapture2::ErrorType::PGRERROR_OK) {
    throw PtGreyException(HL_DEBUG + "Error getting current binning settings"
                          + error.GetDescription());
  }
  ...
  if (h_binning != current_h_binning || v_binning != current_v_binning) {
    ...
    if (error != FlyCapture2::ErrorType::PGRERROR_OK) {
      throw PtGreyException(HL_DEBUG + "Error setting binning settings"
                            + error.GetDescription());
    }
  }
}
...
void FlyCapImageProvider::setPixelFormat(FlyCapture2::PixelFormat pixel_format) {
  ...
  if (error != FlyCapture2::ErrorType::PGRERROR_OK) {
    throw PtGreyException(HL_DEBUG + "Error setting image settings: "
                          + error.GetDescription());
  }
}
